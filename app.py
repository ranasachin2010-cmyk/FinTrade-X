...
.t-h {color:#6b7280; font-size:11px;}
...
<div class="header">
  <div style="display:flex; justify-content:space-between; align-items:center;">
    <div style="display:flex; gap:12px; align-items:center;">
      <div style="width:42px; height:42px; background:linear-gradient(135deg, #a3ff12, #00ff88); border-radius:10px; display:flex; align-items:center; justify-content:center;">◈</div>
      <div><div style="color:#fff; font-size:24px; font-weight:900;">FinTrade <span style="color:#c6ff00;">X</span></div><div style="color:#6b7280; font-size:11px;">Nifty 500 AI • AI-Powered Trading Dashboard</div></div>
    </div>
...
NIFTY500_SAMPLE = ["KOTAKBANK","TITAN","INFY","HINDUNILVR","MARUTI","ASIANPAINT","WIPRO","HCLTECH","SUNPHARMA","RELIANCE"]
...
with c1:
    ...
    <div class="glass" style="margin-top:12px;">
      ...
      <div class="t-h">Today's P&L</div><div style="color:#00ff88; font-size:20px; font-weight:800;">+₹12,450</div>
      <div style="display:flex; justify-content:space-between; margin-top:14px;">
        <div><div class="t-h">Total Value</div><div class="t-v">₹9,43,280</div></div>
        <div><div class="t-h">Win Rate</div><div style="color:#00ff88; font-weight:800;">68%</div></div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if len(top10) >= 2:
        s1 = top10[0]
        s2 = top10[1]
        s1_name = s1.get("STOCK")
        s2_name = s2.get("STOCK")
        s1_price = s1.get("PRICE")
        s2_price = s2.get("PRICE")
        s1_score = s1.get("SCORE")
        s2_score = s2.get("SCORE")
        s1_sig = s1.get("SIGNAL")
        s2_sig = s2.get("SIGNAL")
        s1_rsi = s1.get("RSI")
        s2_rsi = s2.get("RSI")
        b1 = "b-buy" if s1_sig=="BUY" else "b-sell" if s1_sig=="SELL" else "b-hold"
        b2 = "b-buy" if s2_sig=="BUY" else "b-sell" if s2_sig=="SELL" else "b-hold"

        st.markdown(f"""
        <div class="glass" style="margin-top:12px;">
          <div style="color:#fff; font-weight:700; font-size:13px; margin-bottom:10px;">📶 AI SIGNALS</div>
          <div style="display:flex; gap:8px; align-items:center; margin-bottom:12px;">
            <span class="{b1}">{s1_sig}</span>
            <div><div style="color:#fff; font-size:11px; font-weight:700;">{s1_name}</div><div style="color:#6b7280; font-size:10px;">Score {s1_score} • Rs {s1_price} • RSI {s1_rsi}</div></div>
          </div>
          <div style="display:flex; gap:8px; align-items:center;">
            <span class="{b2}">{s2_sig}</span>
            <div><div style="color:#fff; font-size:11px; font-weight:700;">{s2_name}</div><div style="color:#6b7280; font-size:10px;">Score {s2_score} • Rs {s2_price} • RSI {s2_rsi}</div></div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with c2:
    ...
    fig.add_scatter(x=hist.index, y=hist['Close'].rolling(20).mean(), mode='lines', line=dict(color='#00ff88', width=1.5))
    ...
    sel_name = sel["STOCK"] if sel else "NIFTY 500"
    sel_rsi = sel["RSI"] if sel else 63.4
    st.markdown(f"""<div class="glass"><div style="color:#fff; font-weight:800; font-size:14px; margin-bottom:6px;">{sel_name} • 1D CHART</div>""", unsafe_allow_html=True)
    st.plotly_chart(fig, use_container_width=True)
    st.markdown(f"""<div style="background:#12161f; border-radius:8px; padding:6px 12px; color:#9ca3af; font-size:11px;">RSI: {sel_rsi} Neutral-Bullish</div></div>""", unsafe_allow_html=True)
    ...
    for i, r in enumerate(top10, 1):
        ...
        rows_html += f"""<div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid rgba(255,255,255,0.05); color:#fff; font-size:12px;"><div style="width:180px;"><span style="color:#6b7280;">{i:02d}</span> ● {r["STOCK"]}</div><div style="width:80px;">₹{r["PRICE"]}</div><div style="width:70px; color:{color};">{r["CHANGE"]:+.2f}% {arrow}</div><div style="width:50px;"><span class="b-score">{r["SCORE"]}</span></div><div style="width:50px;"><span class="{badge}">{r["SIGNAL"]}</span></div></div>"""

    st.markdown(f"""
    <div class="glass" style="margin-top:12px;">
      <div style="display:flex; justify-content:space-between; color:#fff; font-weight:800; font-size:13px; margin-bottom:12px;">≡ TOP 10 STOCKS — NIFTY 500</div>
      <div style="display:flex; justify-content:space-between; color:#6b7280; font-size:10px; padding-bottom:6px; border-bottom:1px solid #1e2332;"><div style="width:180px;"># STOCK</div><div style="width:80px;">PRICE</div><div style="width:70px;">CHANGE</div><div style="width:50px;">SCORE</div><div style="width:50px;">SIGNAL</div></div>
      {rows_html}
    </div>
    """, unsafe_allow_html=True)
