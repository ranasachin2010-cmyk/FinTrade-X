...
symbols_input = st.sidebar.text_input("Stocks (comma se alag karo)", "RELIANCE, ATGL, TCS")
...
def ai_signal(df):
    ...
    if rsi < 30:
        score += 2
        reasons.append("RSI Oversold (<30) - Buy chance")
    elif rsi > 70:
        score -= 2
        reasons.append("RSI Overbought (>70) - Sell pressure")
    else:
    ...
    if price > ema20:
        score += 1
        reasons.append("Price EMA20 ke upar")
    else:
        score -= 1
        reasons.append("Price EMA20 ke neeche")
    ...
    if df.empty:
        ...
        with st.spinner("News check kar rahe hain..."):
            ...
            if not titles:
                ...
                if pos > neg:
                    st.success("Sentiment: Positive 🟢 - Market me achhi khabar hai")
                elif neg > pos:
                    st.error("Sentiment: Negative 🔴 - Savdhaan raho")
                else:
